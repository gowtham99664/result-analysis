...
def admin_overview_stats():
    ...
    try:
        ...
        # 4. Build pass/fail statistics
        # Per year/semester pass/fail counts (a student passes a semester if they have 0 FAIL/AB/MP subjects)
        pass_fail_stats = {}
        cursor.execute(
            """SELECT r.year, r.semester,
                      COUNT(DISTINCT r.roll_number) as total_students,
                      COUNT(DISTINCT CASE WHEN r.status IN ('FAIL', 'AB', 'MP') THEN r.roll_number END) as failed_students
               FROM results r
        ...
        for row in cursor.fetchall():
            key = f"{row['year']}-{row['semester']}"
            total = row["total_students"]
            failed = row["failed_students"]
            passed = total - failed
            pass_fail_stats[key] = {
                "year": row["year"],
                "semester": row["semester"],
        ...
        # Per branch per year/semester pass/fail
        branch_pass_fail = {}
        cursor.execute(
            """SELECT s.branch, r.year, r.semester,
                      COUNT(DISTINCT r.roll_number) as total_students,
                      COUNT(DISTINCT CASE WHEN r.status IN ('FAIL', 'AB', 'MP') THEN r.roll_number END) as failed_students
               FROM results r
               JOIN students s ON r.roll_number = s.roll_number
               GROUP BY s.branch, r.year, r.semester
               ORDER BY s.branch, r.year, r.semester"""
        )
        for row in cursor.fetchall():
            branch = row["branch"] or "OTHER"
            if branch not in branch_pass_fail:
                branch_pass_fail[branch] = {}
            key = f"{row['year']}-{row['semester']}"
            total = row["total_students"]
            failed = row["failed_students"]
            passed = total - failed
            branch_pass_fail[branch][key] = {
                "year": row["year"],
                "semester": row["semester"],
                "total": total,
                "passed": passed,
        ...
        # Per batch per year/semester pass/fail
        batch_pass_fail = {}
        for batch_year, roll_numbers in sorted(batch_students.items()):
            if not roll_numbers:
                continue
            placeholders = ",".join(["%s"] * len(roll_numbers))
            cursor.execute(
                f"""SELECT r.year, r.semester,
                           COUNT(DISTINCT r.roll_number) as total_students,
                           COUNT(DISTINCT CASE WHEN r.status IN ('FAIL', 'AB', 'MP') THEN r.roll_number END) as failed_students
                    FROM results r
                    WHERE r.roll_number IN ({placeholders})
                    GROUP BY r.year, r.semester
                    ORDER BY r.year, r.semester""",
                roll_numbers,
            )
            ...
            if batch_sems:
                try:
                    end_yr = int(batch_year) + 4
                    b_label = f"{batch_year}-{end_yr}"
                except (ValueError, TypeError):
                    b_label = batch_year
                batch_pass_fail[batch_year] = {
                    "batch_label": b_label,
                    "semesters": batch_sems,
                }
            ...
            jsonify(
                {
                    ...
                    "branch_pass_fail": branch_pass_fail,
...
def get_filtered_students():
    """Get students with optional branch/section/year/semester filters (admin only)."""
    claims = get_jwt()
    if claims.get("role") != "admin":
        return jsonify({"error": "Access denied"}), 403
    ...
    try:
        ...
        # If year or semester filter is provided, JOIN with results to find students
        # who have results for that year/semester
        if year or semester:
            query = """SELECT DISTINCT s.id, s.full_name, s.roll_number, s.branch, s.section, s.created_at
                       FROM students s
        ...
        if branch:
            query += (
                " AND branch = %s" if not (year or semester) else " AND s.branch = %s"
            )
            params.append(branch)
        if section:
            query += (
                " AND section = %s" if not (year or semester) else " AND s.section = %s"
            )
