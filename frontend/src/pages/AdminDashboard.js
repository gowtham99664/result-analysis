...
function AdminDashboard() {
  ...
  const [studentFilterSemester, setStudentFilterSemester] = useState('');
  ...
  const [cgpaFilterYear, setCgpaFilterYear] = useState('');
  const [cgpaFilterSemester, setCgpaFilterSemester] = useState('');
  const [filteredCgpa, setFilteredCgpa] = useState(null);
  const [filteredCgpaLoading, setFilteredCgpaLoading] = useState(false);
  const [filteredCgpaLabel, setFilteredCgpaLabel] = useState('');
  ...
  const switchTab = (tab) => {
    ...
    setCgpaFilterYear('');
    setCgpaFilterSemester('');
    setFilteredCgpa(null);
    setFilteredCgpaLabel('');
  ...
  const handleCalculateFilteredCgpa = async () => {
    if (!cgpaFilterYear || !cgpaFilterSemester) {
      toast.error('Please select both Year and Semester');
      return;
    }
    if (!viewingStudent) return;
    setFilteredCgpaLoading(true);
    try {
      const response = await axios.get(
        `${API_URL}/admin/student-results/${viewingStudent.roll_number}?year=${cgpaFilterYear}&semester=${cgpaFilterSemester}`,
        { headers }
      );
      setFilteredCgpa(response.data.cgpa);
      setFilteredCgpaLabel(`Up to Year ${YEAR_LABELS[cgpaFilterYear]} - Sem ${SEM_LABELS[cgpaFilterSemester]}`);
    } catch (error) {
      toast.error('Failed to calculate CGPA');
    } finally {
      setFilteredCgpaLoading(false);
    }
  };

  const handleClearFilteredCgpa = () => {
    setCgpaFilterYear('');
    setCgpaFilterSemester('');
    setFilteredCgpa(null);
    setFilteredCgpaLabel('');
      ...
      <div className="dashboard-container">
        ...
        {activeTab === 'dashboard' && (
          <>
            ...
            {overviewStats?.pass_fail_stats && Object.keys(overviewStats.pass_fail_stats).length > 0 && (
              <>
                {/* Overall Pass/Fail by Year-Semester */}
                <div className="table-container" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
                  <h3 style={{ color: 'var(--primary-color)', marginBottom: '1rem', fontSize: '1rem' }}>Pass/Fail Analysis by Year-Semester</h3>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={Object.keys(overviewStats.pass_fail_stats).sort().map(key => {
                      const d = overviewStats.pass_fail_stats[key];
                      return {
                        name: `Y${YEAR_LABELS[d.year] || d.year}-S${SEM_LABELS[d.semester] || d.semester}`,
                        Passed: d.passed,
                        Failed: d.failed,
                        'Pass %': d.pass_pct,
                      };
                      ...
                      <XAxis dataKey="name" fontSize={12} />
                      <YAxis fontSize={12} />
                      <Tooltip formatter={(value, name) => name === 'Pass %' ? `${value}%` : value} />
                      <Legend />
                  ...
                  <table style={{ marginTop: '1rem' }}>
                    <thead>
                      <tr>
                        <th>Year-Semester</th><th>Total</th><th>Passed</th><th>Failed</th><th>Pass %</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.keys(overviewStats.pass_fail_stats).sort().map(key => {
                        const d = overviewStats.pass_fail_stats[key];
                        return (
                          <tr key={key}>
                            <td>Year {YEAR_LABELS[d.year] || d.year} - Sem {SEM_LABELS[d.semester] || d.semester}</td>
                            <td>{d.total}</td>
                            <td><strong style={{ color: '#2e7d32' }}>{d.passed}</strong></td>
                            <td><strong style={{ color: '#c62828' }}>{d.failed}</strong></td>
                            <td>
                              <span className="sgpa-badge" style={{ background: d.pass_pct >= 80 ? '#e8f5e9' : d.pass_pct >= 50 ? '#fff3e0' : '#ffebee', color: d.pass_pct >= 80 ? '#2e7d32' : d.pass_pct >= 50 ? '#e65100' : '#c62828' }}>
                                {d.pass_pct}%
                              </span>
                            </td>
                          </tr>
                ...
                {/* Branch-wise Pass/Fail */}
                {overviewStats.branch_pass_fail && Object.keys(overviewStats.branch_pass_fail).length > 0 && (
                  <div className="table-container" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
                    <h3 style={{ color: 'var(--primary-color)', marginBottom: '1rem', fontSize: '1rem' }}>Branch-wise Pass/Fail Analysis</h3>
                    <div className="charts-section" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '1.5rem' }}>
                      {Object.keys(overviewStats.branch_pass_fail).sort().map(branch => {
                        const branchData = overviewStats.branch_pass_fail[branch];
                        const chartData = Object.keys(branchData).sort().map(key => {
                        ...
                        return (
                          <div key={branch} style={{ border: '1px solid #eee', borderRadius: '8px', padding: '1rem' }}>
                            <h4 style={{ color: '#333', marginBottom: '0.5rem', fontSize: '0.95rem' }}>{branch}</h4>
                            <ResponsiveContainer width="100%" height={200}>
                              <BarChart data={chartData}>
                                ...
                                <Bar dataKey="Failed" fill="#c62828" />
                              </BarChart>
                            </ResponsiveContainer>
                          </div>
                        );
                ...
                {/* Batch-wise Pass/Fail */}
                {overviewStats.batch_pass_fail && Object.keys(overviewStats.batch_pass_fail).length > 0 && (
                  <div className="table-container" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
                    <h3 style={{ color: 'var(--primary-color)', marginBottom: '1rem', fontSize: '1rem' }}>Batch-wise Pass/Fail Analysis</h3>
                    {Object.keys(overviewStats.batch_pass_fail).sort().map(batchYear => {
                      const batchData = overviewStats.batch_pass_fail[batchYear];
                      const chartData = Object.keys(batchData.semesters).sort().map(key => {
                        const d = batchData.semesters[key];
                        return {
                          name: `Y${YEAR_LABELS[d.year] || d.year}-S${SEM_LABELS[d.semester] || d.semester}`,
                          Passed: d.passed,
                          Failed: d.failed,
                      ...
                      return (
                        <div key={batchYear} style={{ marginBottom: '1.5rem', border: '1px solid #eee', borderRadius: '8px', padding: '1rem' }}>
                          <h4 style={{ color: '#333', marginBottom: '0.5rem', fontSize: '0.95rem' }}>
                            Batch {batchData.batch_label}
                          </h4>
                          ...
                          <table style={{ marginTop: '0.5rem', fontSize: '0.9rem' }}>
                            <thead>
                              <tr><th>Semester</th><th>Total</th><th>Passed</th><th>Failed</th><th>Pass %</th></tr>
                            </thead>
                            <tbody>
                              {Object.keys(batchData.semesters).sort().map(key => {
                                const d = batchData.semesters[key];
                                return (
                                  <tr key={key}>
                                    <td>Year {YEAR_LABELS[d.year] || d.year} - Sem {SEM_LABELS[d.semester] || d.semester}</td>
                                    <td>{d.total}</td>
                                    <td style={{ color: '#2e7d32', fontWeight: 'bold' }}>{d.passed}</td>
                                    <td style={{ color: '#c62828', fontWeight: 'bold' }}>{d.failed}</td>
                                    <td>
                                      <span className="sgpa-badge" style={{ background: d.pass_pct >= 80 ? '#e8f5e9' : d.pass_pct >= 50 ? '#fff3e0' : '#ffebee', color: d.pass_pct >= 80 ? '#2e7d32' : d.pass_pct >= 50 ? '#e65100' : '#c62828', fontSize: '0.8rem' }}>
                                        {d.pass_pct}%
                                      </span>
                                    </td>
                                  </tr>
          ...
          <>
            {!viewingStudent ? (
              <>
                ...
                <div className="filter-section">
                  <div className="filter-row">
                    ...
                    <div className="filter-group">
                      <label>Year</label>
                      <select className="form-control" value={studentFilterYear} onChange={(e) => setStudentFilterYear(e.target.value)}>
                    ...
                    <div className="filter-group">
                      ...
                      <select className="form-control" value={studentFilterSemester} onChange={(e) => setStudentFilterSemester(e.target.value)}>
              ...
              <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                    ...
                    {viewingStudentCgpa !== null && viewingStudentCgpa !== undefined && (
                      <div style={{ textAlign: 'center', background: 'linear-gradient(135deg, var(--primary-color), var(--secondary-color))', color: '#fff', padding: '1rem 2rem', borderRadius: '12px' }}>
                        <div style={{ fontSize: '0.8rem', opacity: 0.9 }}>Overall CGPA</div>
                ...
                {/* CGPA Calculator — up to selected Year/Semester */}
                <div className="table-container" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
                  <h3 style={{ color: 'var(--primary-color)', marginBottom: '1rem', fontSize: '1rem' }}>CGPA Calculator</h3>
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1rem', flexWrap: 'wrap' }}>
                    <div className="filter-group">
                      <label style={{ fontSize: '0.85rem', color: '#555', marginBottom: '0.25rem', display: 'block' }}>Year</label>
                      <select className="form-control" value={cgpaFilterYear} onChange={(e) => { setCgpaFilterYear(e.target.value); setFilteredCgpa(null); setFilteredCgpaLabel(''); }} style={{ minWidth: '120px' }}>
                    ...
                    <div className="filter-group">
                      <label style={{ fontSize: '0.85rem', color: '#555', marginBottom: '0.25rem', display: 'block' }}>Semester</label>
                      <select className="form-control" value={cgpaFilterSemester} onChange={(e) => { setCgpaFilterSemester(e.target.value); setFilteredCgpa(null); setFilteredCgpaLabel(''); }} style={{ minWidth: '140px' }}>
                    ...
                    <button
                      ...
                      onClick={handleCalculateFilteredCgpa}
                      disabled={filteredCgpaLoading || !cgpaFilterYear || !cgpaFilterSemester}
                      style={{ height: '38px', padding: '0 1.25rem' }}
                    >
                      {filteredCgpaLoading ? 'Calculating...' : 'Calculate'}
                    </button>
                    {filteredCgpa !== null && (
                      <button
                        className="btn btn-secondary btn-sm"
                        onClick={handleClearFilteredCgpa}
                        style={{ height: '38px', padding: '0 1rem', color: '#666', borderColor: '#ccc' }}
                      >
                        Clear
                      </button>
                    )}
                  ...
                  {filteredCgpa !== null && (
                    <div style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <div style={{ textAlign: 'center', background: 'linear-gradient(135deg, #2e7d32, #43a047)', color: '#fff', padding: '0.75rem 1.5rem', borderRadius: '12px', display: 'inline-block' }}>
                        <div style={{ fontSize: '0.75rem', opacity: 0.9 }}>CGPA {filteredCgpaLabel}</div>
                        <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>{filteredCgpa}</div>
                      </div>
                      {viewingStudentCgpa !== null && viewingStudentCgpa !== undefined && (
                        <div style={{ fontSize: '0.85rem', color: '#888' }}>
                          Overall CGPA: <strong>{viewingStudentCgpa}</strong>
                        </div>
                      )}
                    </div>
                  )}
                  {filteredCgpa === 0 && cgpaFilterYear && cgpaFilterSemester && !filteredCgpaLoading && (
                    <div style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: '#f44336' }}>
                      No results found for this student up to Year {YEAR_LABELS[cgpaFilterYear]} - Sem {SEM_LABELS[cgpaFilterSemester]}.
                    </div>
                  )}
                </div>
                ...
                <table>
                  <thead>
                    <tr>
                      ...
                      <th>Filename</th><th>Year/Semester Data</th><th>Subjects</th>
                  ...
                  <tbody>
                    {uploads.map((upload, idx) => {
                      let ysList = [];
                      try {
                        const parsed = typeof upload.year_semester_data === 'string'
                          ? JSON.parse(upload.year_semester_data)
                          : upload.year_semester_data;
                        if (Array.isArray(parsed)) {
                          ysList = parsed;
                        } else if (parsed && typeof parsed === 'object') {
                          ysList = Object.entries(parsed).map(([k, v]) => {
                            const parts = k.split('-');
                            return { year: parts[0], semester: parts[1], subjects: v };
                          });
                        }
                        ...
                        <tr key={upload.id}>
                          ...
                          <td>
                            {ysList.length > 0 ? (
                              ...
                              <span style={{ color: '#999' }}>{upload.num_semesters} sem(s)</span>
                            )}
                          </td>
